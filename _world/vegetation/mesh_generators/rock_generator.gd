...
## Generate an ore boulder - larger mineable rock with ore material
static func generate_ore_boulder(ore_type: int, seed_value: int, lod_level: int = 0) -> ArrayMesh:
	var rng := RandomNumberGenerator.new()
	rng.seed = seed_value
	
	# Ore boulders are larger (1.5 - 3.0 meters)
	var size := rng.randf_range(1.5, 3.0)
	
	# Color based on ore type (using VegetationManager enum values)
	var color: Color
	match ore_type:
		6:  # ORE_IRON
			color = Color(0.4, 0.35, 0.32)  # Dark iron-rust color
			color = color.lerp(Color(0.6, 0.45, 0.35), rng.randf_range(0.0, 0.4))  # Rust spots
		7:  # ORE_COPPER
			color = Color(0.6, 0.45, 0.3)  # Copper-orange tint
			color = color.lerp(Color(0.3, 0.55, 0.45), rng.randf_range(0.0, 0.3))  # Green patina
		_:  # ORE_STONE (default)
			color = Color(0.55, 0.52, 0.5)  # Gray stone
	
	# Add color variation
	color = color * rng.randf_range(0.9, 1.1)
