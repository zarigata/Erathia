...
func _process(delta):
	...
	# We don't hard-check collider type anymore because terrain can be either:
	# - custom Chunk-based terrain (collider is Chunk)
	# - VoxelLodTerrain (collider is not Chunk)
	# Collision mask should already filter to terrain.
	if collider == null:
		return
...
func _handle_primary_action():
	...
	if current_tool == MiningSystem.ToolType.HOE or current_tool == MiningSystem.ToolType.STAFF:
		# Smoothing Mode
		if terrain_manager.has_method("smooth_terrain"):
		...
		if terrain_manager.has_method("get_material_at"):
			mat_id = terrain_manager.get_material_at(dig_point)
...
func _handle_secondary_action():
	...
	if current_tool == MiningSystem.ToolType.HOE or current_tool == MiningSystem.ToolType.STAFF:
		...
		if terrain_manager.has_method("flatten_terrain"):
